...
import type { QuizTopic, QuizLevel } from '../quizAlj1';
import { QUIZ_B } from '../quizAlj2';
import { getStars, setStar, getSettings, sfxClick, sfxWin, sfxFail, submitScore, sfxStar, speak, beep, bindKeys, recordMistake, recordOk, weakKey } from '../util';
import { mascotBadge } from '../mascot';
import { ensureFxTextures, neonBackdrop, burst } from '../fx';
import { pal } from '../theme';

// Quiz generik Aljabar Linier — topik dipilih via data {topic}.
// Tiap level 3 soal, 3 nyawa, 15 detik per soal. levelId 0 = pilih level.

const TOPICS: Record<string, QuizTopic> = { ...QUIZ_A, ...QUIZ_B };
const TIME_MS = 15000;
const OPT_Y = [600, 722, 844];

export class QuizScene extends Phaser.Scene {
  private topicId = '';
  private topic: QuizTopic | null = null;
  private levelId = 0;
  private lv: QuizLevel | null = null;
  private qi = 0;
  private hearts = 3;
  private mistakes = 0;
  private streak = 0;
  ...
  init(data: { topic?: string; levelId?: number }): void {
    this.topicId = data.topic ?? '';
    this.topic = TOPICS[this.topicId] ?? null;
    this.levelId = data.levelId ?? 0;
    const found = this.topic?.levels.find((l) => l.id === this.levelId) ?? null;
    this.lv = found;
    this.qi = 0;
    this.hearts = 3;
    this.mistakes = 0;
  ...
  private buildSelect(): void {
    ...
    this.add
      ...
      .setOrigin(0.5);
    this.add
      .text(W / 2, 152, tp.sub, { fontSize: '22px', color: P.dim })
      .setOrigin(0.5);

    const how = this.add.rectangle(W / 2, 250, 624, 120, 0x0b1628, 0.9).setOrigin(0.5);
    how.setStrokeStyle(2, 0x1e3a5f, 1);
    this.add
      .text(W / 2, 250, tp.howto, { fontSize: '22px', color: '#e2e8f0', align: 'center', lineSpacing: 7 })
      .setOrigin(0.5);

    const stars = getStars(tp.id);
    let next = 1;
    for (let i = 1; i <= tp.levels.length; i++) {
      if (stars[i]) next = Math.min(tp.levels.length, i + 1);
    }
    const cols = 4;
    const cw = 140;
    const ch = 120;
    ...
    tp.levels.forEach((lv, i) => {
      ...
      z.on('pointerdown', () => {
        sfxClick();
        this.scene.restart({ topic: tp.id, levelId: lv.id });
      });
    });

    this.add
      .text(W / 2, 700, tp.keywords, {
    ...
    const keys: Record<string, () => void> = {
      ...
      ENTER: () => this.scene.restart({ topic: tp.id, levelId: next }),
    };
    tp.levels.forEach((lv, i) => {
      if (i < numNames.length) keys[numNames[i]] = () => this.scene.restart({ topic: tp.id, levelId: lv.id });
    });
    bindKeys(this, keys);
  }
  ...
  private buildPlay(): void {
    ...
    back.on('pointerdown', () => {
      sfxClick();
      this.scene.restart({ topic: tp.id, levelId: 0 });
    });
    this.add
    ...
    this.qTx = this.add
      .text(W / 2, 330, '', {
        fontSize: '25px', color: '#f8fafc', fontStyle: 'bold', align: 'center',
        lineSpacing: 7, wordWrap: { width: 540 },
      })
      .setOrigin(0.5);
    ...
    for (let i = 0; i < 3; i++) {
      ...
      const tx = this.add
        .text(W / 2, y, '', { fontSize: '27px', color: '#f8fafc', fontStyle: 'bold', align: 'center', wordWrap: { width: 560 } })
        .setOrigin(0.5);
    ...
    this.explTx = this.add
      .text(W / 2, 940, '', {
        fontSize: '21px', color: '#e2e8f0', align: 'center', lineSpacing: 6,
  ...
  private choose(i: number): void {
    ...
    this.timerOn = false;
    if (i === q.ans) {
      recordOk(weakKey(this.topicId, this.lv.id, this.qi));
      this.streak += 1;
      this.paintStreak();
  ...
  private onWrong(prefix: string, picked: number): void {
    const q = this.lv!.questions[this.qi];
    recordMistake(weakKey(this.topicId, this.lv!.id, this.qi));
    this.mistakes += 1;
    this.streak = 0;
    this.paintStreak();
    this.hearts -= 1;
  ...
  private onWin(): void {
    ...
    setStar(tp.id, lv.id, earned);
    submitScore(tp.id, lv.id, earned);
  ...
  private endOverlay(title: string, stars: string, sub: string, isLast: boolean, failed = false): void {
    ...
    if (failed) {
      ...
      mkBtn(656, isLast ? '★ SELESAI — KE TOPIK' : '▶ LEVEL BERIKUTNYA', 0x22d3ee, () => {
        sfxClick();
        if (isLast) this.scene.start('AljTopik');
        else this.scene.restart({ topic: tp.id, levelId: lv.id + 1 });
      });
