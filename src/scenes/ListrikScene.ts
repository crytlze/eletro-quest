...
import { LIST_LEVELS, ListLevel } from '../listLevels';
...
// Dasar Kelistrikan — kuis konsep: satuan, alat ukur, bahan, Ohm, daya.
...
export class ListrikScene extends Phaser.Scene {
  private levelId = 0;
  private lv: ListLevel | null = null;
  private qi = 0;
  private hearts = 3;
  private mistakes = 0;
  private streak = 0;
  ...
  init(data: { levelId?: number }): void {
    ...
    const found = LIST_LEVELS.find((l) => l.id === this.levelId);
    this.lv = found ?? null;
    this.qi = 0;
    this.hearts = 3;
    this.mistakes = 0;
  ...
  private buildSelect(): void {
    ...
    this.add
      .text(W / 2, 100, 'DASAR KELISTRIKAN', {
      ...
      .setOrigin(0.5);
    this.add
      .text(W / 2, 152, 'Konsep • satuan • alat ukur', { fontSize: '22px', color: P.dim })
      .setOrigin(0.5);
    ...
    let next = 1;
    for (let i = 1; i <= LIST_LEVELS.length; i++) {
      if (stars[i]) next = Math.min(LIST_LEVELS.length, i + 1);
    }
    const cols = 4;
    const cw = 140;
    const ch = 120;
    const stepX = (624 - cw) / (cols - 1);
    LIST_LEVELS.forEach((lv, i) => {
      const col = i % cols;
    ...
    this.add
      .text(W / 2, 700, 'Satuan • alat ukur • bahan • Ohm • daya', {
  ...
  private choose(i: number): void {
    ...
    this.timerOn = false;
    if (i === q.ans) {
      recordOk(weakKey('listrik', this.lv.id, this.qi));
      this.streak += 1;
      this.paintStreak();
  ...
  private onWrong(prefix: string, picked: number): void {
    const q = this.lv!.questions[this.qi];
    recordMistake(weakKey('listrik', this.lv!.id, this.qi));
    this.mistakes += 1;
    this.streak = 0;
    this.paintStreak();
    this.hearts -= 1;
  ...
  private onWin(): void {
    ...
    submitScore('listrik', lv.id, earned);
